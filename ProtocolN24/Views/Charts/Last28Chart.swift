...
struct Last28Chart: View {
    ...
    var body: some View {
        ZStack {
            ...
            .chartXScale(domain: [Calendar.current.date(byAdding: .day, value: -29, to: Date())!,
    ...
    func filterLast14() {
        let filteredWeighIns = weighIns.filter { weighIn in
            if weighIn.date > Calendar.current.date(byAdding: .day, value: -29, to: Date())! {
                return true
            } else { return false }
        }
        last14WeighIns = filteredWeighIns.sorted()
        last14Series.name = "Last28"
